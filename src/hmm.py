import os
from universal_utils import get_data, read_binary_file
...
class HMM:
    ...
    def train_model(self):
        ...
        train_data = get_data(self.training_file)
        ...
        plt.title('number of states: %d' % self.num_of_state)
        plt.savefig(os.path.join("imgs", "log_likelihood_%d" % self.num_of_state) + ".png")
    ...
    def fit(self, train_data, it, progress_bar):
        ...
        files = [data[1] for data in train_data]
        k = [data[0] - 1 for data in train_data]
        length = len(k)
        trained = 1
        with concurrent.futures.ProcessPoolExecutor(max_workers=15) as executor:
            for res in executor.map(EM_HMM_FR, mean_by_k, var_by_k, aij_by_k, files, k):
    ...
    def EM_initialization_model(self):
        ...
        for u in range(0, num_of_train):
            filename = train_data[u][1]
            if os.path.exists(filename):
                feature = read_binary_file(filename)
    ...
    def test_model(self):
        ...
        num_of_error = 0
        num_of_testing = 0
        test_data = get_data(self.testing_file)
        # 总测试数据个数
        ...
        files = [data[1] for data in test_data]
        with concurrent.futures.ProcessPoolExecutor(max_workers=15) as executor:
            for pre in executor.map(self._test, files, labels):
                if not pre:
                    num_of_error += 1
